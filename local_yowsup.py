...
class RegistrationArgParser(YowArgParser):
    ...
    def process(self):
        ...
        if self.args["requestcode"]:
            print("self.args['requestcode']", self.args['requestcode'])
            print('config ______', config)
    ...
    def handleRequestCode(self, method, config):
        ...
        if not self._preview:
            ...
            os.environ['send_message_result'] = json.dumps(result)
        else:
            print(config)
    ...
    def handleRegister(self, code, config):
        ...
        if not self._preview:
            if result["status"] == "ok":
                ...
                if "chat_dns_domain" in result:
                    self._config.chat_dns_domain = result["chat_dns_domain"]
                if 'sms_code_error' in os.environ:
                    os.environ['sms_code_error'] = 'False'
                source = './' if 'json_source' not in os.environ else os.environ['json_source'] 
                source = os.path.join(source, os.environ['phone'] + '.json')
                self._config_manager.save(self._config, dest=source)

            elif 'status' in result and result['status'] == 'fail':
                os.environ['sms_code_error'] = 'error'
